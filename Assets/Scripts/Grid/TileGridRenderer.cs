...
//}
#endregion
#region Phase 1 Sprint 6b - Tile Grid Rendering with Tilemap Alignment
//using UnityEngine;
...
//        switch (tile.tileType)
//        {
//            case TileType.Empty: sr.color = Color.clear; break;
//            case TileType.Occupied: sr.color = WithAlpha(colorOccupied); break;
//            case TileType.NPCCamp: sr.color = WithAlpha(colorNPCCamp); break;
//            case TileType.Resource: sr.color = WithAlpha(colorResource); break;
//        }
...
//}
#endregion
#region Phase 1 Sprint 9 - Tile Grid Rendering with Tilemap Alignment and Debug Toggle
using UnityEngine;
...
public class TileGridRenderer : MonoBehaviour
{
    ...
    public void SetTileColor(int x, int y, Color color)
    {
        ...
        SpriteRenderer sr = tileRenderers[x, y];
        if (sr == null) return;
        sr.enabled = true; // Always show preview regardless of showGrid
        sr.color = color;
    }
    ...
    public void RefreshTile(int x, int y)
    {
        ...
        sr.enabled = showGrid; // Respect showGrid when restoring
        ...
        for (int x = 0; x < gridWidth; x++)
        {
            for (int y = 0; y < gridHeight; y++)
            {
                if (tileRenderers[x, y] != null)
                    tileRenderers[x, y].enabled = showGrid;
    ...
    Color WithAlpha(Color c)
    {
        return new Color(c.r, c.g, c.b, c.a * gridAlpha);
    }
